...
\begin{itemize}
\item His estimate is unbiased if the change in $Y^0$ for the comparison cities correctly approximates the unobserved $\textcolor{violet-red}{\Delta Y^0}$ for the treatment group
...
\begin{frame}{KYLE EDIT: Factor Model Intuition}
  Untreated potential outcomes are given by a factor model:
  \begin{equation}\label{eq:untreated_po}
    Y_{it}^0 = \alpha_t + \textcolor{navy}{\lambda_t} \textcolor{orange}{u_i} + u_{it}
  \end{equation}
...
\begin{frame}{KYLE EDIT: Two-way Fixed Effect vs. Factor Model}
  The factor model is a generalization of the TWFE model. If $\lambda_{t} = (\mu_t, 1)'$ and $u_i = (1, \mu_i)'$, then (\ref{eq:untreated_po}) becomes:
  $$
...
\begin{itemize}
  ...
  \item Only units that are alike in observables and unobservables (factor-loadings!) should produce similar trajectories of the outcome variable over extended periods of time
  \item Proof in Appendix B of ADH (2011)
\end{itemize}

\end{frame}

\begin{frame}{KYLE EDIT: Synthetic Control Bias Under Factor Model}
  Synthetic control will do well when you have many time-periods because:
  \begin{enumerate}
    \item You will not fit on the error term $\epsilon_{it}$ 
    \item Instead, you will find a synthetic control with the same \emph{exposure} to macro-economic shocks (matching on factor-loadings)
  \end{enumerate}

  \bigskip
  More on few time-periods / imperfect fit later...
\end{frame}
